...
import "../assets/css/LongJobCard.css";
import NoJobAvail from "./NoJobAvail";

function LongJobCard({
  filteredJobs,
  getAllJobs,
  remotiveJobOnly,
  adzunaJobOnly,
  ...
  return (
    <>
      ...
      {adzunaJobOnly && adzunaJobs.length === 0 && (
        <p className="text-danger fw-bold">No jobs found from Adzuna.</p>
      )}
      {JSearchJobOnly && JSearchJobs.length === 0 && (
        <p className="text-danger fw-bold">No jobs found from JSearch.</p>
      )}
      {filteredJobs.length === 0 ? (
        <NoJobAvail />
      ) : (
        filteredJobs.map((item) => (
          <div className="long-job-card mb-3" key={item.id}>
            <div className="company-name-and-time d-flex align-items-center justify-content-between">
              <div className="company-name">
                <div className="d-flex align-items-center justify-content-start">
                  <img
                    ...
                    alt=""
                  />
                  <span className="ms-3">{item.companyName}</span>
                </div>
              </div>
              <div className="time">
                <span> {moment(item.datePosted).startOf("day").fromNow()}</span>
              </div>
            </div>
            ...
            <div className="bottom-content mt-2 d-flex align-items-center justify-content-between">
              ...
              <div className="long-card-apply-btn">
